...
import {getOrganization, leaveOrganization} from "@/api/api-calls";
...
export default function CreateOrganization() {
  ...
  const handleLeave = (e) => {
    console.log(leaveOrganization(session.user.key, id))
  ...
  const sidebarOptions = [
    ...
    {id: "leave", name: "Leave", url: "/panel/organization/" + id + "/leave", selected: true, faIcon: faRightFromBracket, faColor: "#ffffff", faWidth: "20px"},
    ...
    <div className={styles.main_container}>
      ...
      <div className={styles.content}>
        ...
        <div className={styles.leave_container}>
          <h2 className={styles.leave_title}>Leave organization</h2>
          <button style={{cursor: 'pointer'}} className={styles.leave_button} onClick={handleLeave}>Leave</button>
        </div>
      </div>
    </div>
  )
